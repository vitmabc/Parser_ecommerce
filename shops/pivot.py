...
def start():
    ...
    def kotel_kr_piramida():
        # Сравнение kotel_kr и piramida
        res_p = merge_table(dataFrame_kotel, dataFrame_piramida, 'sku_kotel_kr', "sku_piramida")
        res_p1 = merge_table(dataFrame_kotel, dataFrame_piramida, 'sku_kotel_kr', "article_piramida")
        res_p = pds.concat([res_p, res_p1])
        res_p = drop_duplicate(res_p, 'id_kotel_kr', 'id_piramida')
        res_p = res_p.reindex(
            columns=['sku_kotel_kr', 'name_kotel_kr', 'article_piramida', 'name_piramida', 'price_kotel_kr',
                     'price_piramida'])
        kotel_kr_lower = res_p[res_p['price_kotel_kr'] < res_p['price_piramida']]
        piramida_lower = res_p[res_p['price_kotel_kr'] > res_p['price_piramida']]
        filename = 'cross_kotel_kr_piramida.xlsx'
        write_excel(res_p, filename, 'cross_table_kotel-piramida')
        write_excel(kotel_kr_lower, filename, 'kotel_kr_lower', mode="a", if_sheet_exists='replace')
        write_excel(piramida_lower, filename, 'piramida_lower', mode="a", if_sheet_exists='replace')
        formats(filename, 'cross_table_kotel-piramida')
        formats(filename, 'kotel_kr_lower')
    ...
    def teplozap_piramida():
        ...
        res_t_p = res_t_p.reindex(
            columns=['sku_teploz', 'name_teploz', 'article_piramida', 'name_piramida', 'price_teploz', 'price_piramida'])
    ...
    def gazkomplekt_piramida():
        # Сравнение gazkomplekt и piramida
        res_g_p = merge_table(dataFrame_gazkomplekt, dataFrame_piramida, 'sku_gazkomplekt', "sku_piramida")
        res_g_p1 = merge_table(dataFrame_gazkomplekt, dataFrame_piramida, 'sku_gazkomplekt', "article_piramida")
        res_g_p = pds.concat([res_g_p, res_g_p1])
        res_g_p = drop_duplicate(res_g_p, 'id_gazkomplekt', 'id_piramida')
        res_g_p = res_g_p.reindex(
            columns=['sku_gazkomplekt', 'name_gazkomplekt', 'article_piramida', 'name_piramida', 'price_gazkomplekt',
                     'price_piramida'])
        gazkomplekt_lower = res_g_p[res_g_p['price_gazkomplekt'] < res_g_p['price_piramida']]
        piramida_lower = res_g_p[res_g_p['price_gazkomplekt'] > res_g_p['price_piramida']]
        filename = 'cross_gazkomplekt_piramida.xlsx'
        write_excel(res_g_p, filename, 'cross_table_gaz-piramida')
        write_excel(gazkomplekt_lower, filename, 'gaz_lower', mode="a", if_sheet_exists='replace')
        write_excel(piramida_lower, filename, 'piramida_lower', mode="a", if_sheet_exists='replace')
        formats(filename, 'cross_table_gaz-piramida')
        formats(filename, 'gaz_lower')
        formats(filename, 'piramida_lower')
    ...
    def kotel_kr_gazkompl():
        # Сравнение kotel_kr и gazkomplekt
        res_t = merge_table(dataFrame_kotel, dataFrame_gazkomplekt, 'sku_kotel_kr', "sku_gazkomplekt")
        res_t = drop_duplicate(res_t, 'id_kotel_kr', 'id_gazkomplekt')
        res_t = res_t.reindex(
            columns=['sku_kotel_kr', 'name_kotel_kr', 'sku_gazkomplekt', 'name_gazkomplekt', 'price_kotel_kr', 'price_gazkomplekt'])

        kotel_kr_lower = res_t[res_t['price_kotel_kr'] < res_t['price_gazkomplekt']]
        gazkompl_lower = res_t[res_t['price_kotel_kr'] > res_t['price_gazkomplekt']]
        filename = 'cross_kotel_kr_gazkomplekt.xlsx'
        write_excel(res_t, filename, 'cross_kotel_kr_gazkomplekt')
        write_excel(kotel_kr_lower, filename, 'kotel_kr_lower', mode="a", if_sheet_exists='replace')
        write_excel(gazkompl_lower, filename, 'gazkompl_lower', mode="a", if_sheet_exists='replace')
        formats(filename, 'cross_kotel_kr_gazkomplekt')
        formats(filename, 'kotel_kr_lower')
        formats(filename, 'gazkompl_lower')
    ...
    # # Запись временных таблиц в SQL чисто информативно
    # pds.D = dataFrame_piramida.to_sql('temp_piramida_new', if_exists='replace', con=dbConnection)
    # pds.D = dataFrame_kotel.to_sql('temp_kotel_kr_new', if_exists='replace', con=dbConnection)
    # pds.D = dataFrame_teploz.to_sql('temp_teplozap_new', if_exists='replace', con=dbConnection)
    # pds.D = dataFrame_gazkomplekt.to_sql('temp_gazkomplekt_new', if_exists='replace', con=dbConnection)
    #
    # # Запись найденных артикулов в рабочую таблицу Пирамиды
    # sql1 = '''UPDATE piramida_catalog as f
    #         SET sku_temp=t.sku_piramida
    #         from temp_piramida_new as t
    #         WHERE f.id = t.id_piramida;
    #         '''
    # with alchemyEngine.begin() as conn:
    #     conn.execute(sql1)
    #
    # sql2 = '''UPDATE gazkomplekt_catalog as f
    #         SET sku_temp=t.sku_gazkomplekt
    #         from temp_gazkomplekt_new as t
    #         WHERE f.id = t.id_gazkomplekt;
    #         '''
    # with alchemyEngine.begin() as conn:
    #     conn.execute(sql2)
